...
const tableList = () => {
  ...
  return datatableHelper(id, "Lista de AP Emisor", {
    ajax: {
      url: ` ${base_url}/apemisor/list_records`,
      dataSrc: "",
    },
    deferRender: true,
    idDataTables: "1",
...
const openCreate = () => {
  document.querySelector("#text-title").innerHTML = "Nuevo AP Emisor";
...
const openUpdate = (id) => {
  document.querySelector("#text-title").innerHTML = "Editar AP Emisor";
  document.querySelector("#text-button").innerHTML = "Actulizar Registro";
  const component = document.getElementById("transactions");
  component.reset();
  axios
    .get(`${base_url}/apemisor/select_record/${id}`)
    .then(({ data }) => {
      $("#transactions").parsley().reset();
      $("#modal-action").modal("show");
...
const update = (evt, id) => {
  ...
  axios
    .post(`${base_url}/apemisor/update_record/${id}`, data)
    .then(({ data }) => {
...
const remove = (id) => {
  return new Promise((resolve, reject) => {
    fetch(`${base_url}/apemisor/remove_record/${id}`, {
      method: "post",
    })
      .then((dataJSON) => dataJSON.json())
      .then((data) => {
