...
const tableList = () => {
  ...
  const getColumns = () => {
    ...
    arrayColumns.push({
      ...
      render: (data, type, full) => {
        ...
        return `<div>${optEdit}${optFiles}${optDelete}</div>`;
      },
    });
    // response
    return arrayColumns;
  ...
  return datatableHelper(id, "Lista de STA Receptor", {
    ajax: {
      url: ` ${base_url}/apreceptor/list_records`,
      dataSrc: "",
    },
    deferRender: true,
    idDataTables: "1",
...
const openCreate = () => {
  document.querySelector("#text-title").innerHTML = "Nuevo STA Receptor";
...
const create = () => {
  ...
  fetch(`${base_url}/apreceptor/save`, {
    method: "post",
    body: formData,
  })
...
const openUpdate = (id) => {
  document.querySelector("#text-title").innerHTML = "Editar STA Receptor";
  document.querySelector("#text-button").innerHTML = "Actualizar Registro";
  const component = document.getElementById("transactions");
  component.reset();
  axios
    .get(`${base_url}/apreceptor/select_record/${id}`)
    .then(({ data }) => {
      $("#transactions").parsley().reset();
      $("#modal-action").modal("show");
...
const update = (evt, id) => {
  ...
  axios
    .post(`${base_url}/apreceptor/update_record/${id}`, data)
    .then(({ data }) => {
...
const remove = (id) => {
  return new Promise((resolve, reject) => {
    fetch(`${base_url}/apreceptor/remove_record/${id}`, {
      method: "post",
    })
      .then((dataJSON) => dataJSON.json())
      .then((data) => {
