<?php
    class Runway2 extends Controllers{
        ...
        public function runway2(){
            if(empty($_SESSION['permits_module']['v'])){
                header("Location:".base_url().'/dashboard');
            }
            $data['page_name'] = "Lista de zonas";
            $data['page_title'] = "Lista de zonas";
        ...
        public function select_record(string $idrunway2){
            if($_SESSION['permits_module']['v']){
                $idrunway2 = decrypt($idrunway2);
                $idrunway2 = intval($idrunway2);
                if($idrunway2 > 0){
                    $data = $this->model->select_record($idrunway2);
                    if(empty($data)){
        ...
        public function list_runway2(){
            $html = "";
            $arrData = $this->model->list_runway2();
            if(count($arrData) > 0){
                for($i=0; $i < count($arrData); $i++) {
                    if($arrData[$i]['state'] == 1 ){
                        $html .= '<option value="'.$arrData[$i]['id'].'">'.$arrData[$i]['nombre_zona'].'</option>';
                    }
                }
            }
            echo $html;
        ...
        public function filter_runway2(){
            $html = "";
            $arrData = $this->model->list_runway2();
            if(count($arrData) > 0){
        ...
        public function action(){
            if($_POST){
                if(empty($_POST['runway2'])){
                    $response = array("status" => 'error', "msg" => 'Campos señalados son obligatorios.');
                }else{
                  $id = decrypt($_POST['idrunway2']);
                  $id = intval($id);
                    $runway2 = strtoupper(strClean($_POST['runway2']));
                    $state = intval(strClean($_POST['listStatus']));
                    $datetime = date("Y-m-d H:i:s");
                    if($id == 0){
                        $option = 1;
                        if($_SESSION['permits_module']['r']){
                            $request = $this->model->create($runway2,$datetime,$state);
                        }
                    }else{
                        $option = 2;
                        if($_SESSION['permits_module']['a']){
                            $request = $this->model->modify($id,$runway2,$state);
                        }
                    }
                    if($request == "success"){
                        if($option == 1){
        ...
        public function remove(){
            if($_POST){
                if($_SESSION['permits_module']['e']){
                    $idrunway2 = decrypt($_POST['idrunway2']);
                    $idrunway2 =  intval($idrunway2);
                    $request = $this->model->remove($idrunway2);
                    if($request == 'success'){
